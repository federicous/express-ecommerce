...
class MongoDB {
	...
	async getAllPage(page,pageSize) {
		try {
			...
			let allProducts = await ProductoModel.find({novedades:"si"}).sort({name:1, color:1, code:1}).skip(skip).limit(PAGE_SIZE);
			let total = await ProductoModel.countDocuments({novedades:"si"})
			// return (allProducts)
